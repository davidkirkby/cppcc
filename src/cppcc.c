...
int main(int argc, char **argv) {
   ...
   double epsilon_r, maxram, ram;
    ...
    if (argc - optind > 9 ) {
        fprintf(stderr, "Error: Too many parameters. Expected: r1 t1 r1 t2 d L W H Er MaxRAM\n");
        fprintf(stderr, "Use the --help or -h option for help\n");
        exit(EXIT_FAILURE);
    }
    if (argc - optind < 9) {
        fprintf(stderr, "Error: Too few parameters. Expected: r1 t1 r1 t2 d L W H Er MaxRAM\n");
        fprintf(stderr, "Use the --help or -h option for help\n");
        exit(EXIT_FAILURE);
    }
